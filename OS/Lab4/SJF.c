...
int main()
{
    ...
    printf("THIS IS THE EXECUTION FOR SHORTEST JOB FIRST SCHEDULING\n\n");
      ...
      for(int ii=0;ii<n;ii++)
    {
        ...
        {
            if(burst_time[jj]<burst_time[ii])
            {
                int temp=burst_time[jj];
                burst_time[jj]=burst_time[ii];
                burst_time[ii]=temp;
                
                int temp2=arrival_time[jj];
                arrival_time[jj]=arrival_time[ii];
                arrival_time[ii]=temp2;                
            }
        }
    }
    ...
    for(i=0;i<n;i++)
    {
        turn_around_time[i]=burst_time[i+1]+waiting_time[i];
